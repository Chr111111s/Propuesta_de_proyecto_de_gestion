    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                actions={
                    <button type="button" className="btn btn-primary rounded-xl" onClick={exportTemplateCsv}>
                        <Download size={16} />
                        Exportar CSV
                    </button>
                }
            ...
            <section className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_340px]">
                            ...
                            <tbody>
                                {templateRows.map((row) => (
                                    <tr key={`${row.folio}-${row.movement_date}`}>
                                        ...
                                        <td className="folio-mono font-bold">{row.folio}</td>
                                        <td>
                ...
                <SectionCard title="Indicadores de plantilla" description="Señales rápidas para negociación y supervisión operativa.">
                    <div className="space-y-4">
                        <div className="rounded-xl border border-base-200 bg-base-200/30 p-5">
                            <p className="text-sm font-semibold uppercase tracking-[0.08em] text-base-content/55">Promedio por registro</p>
                            <p className="display-num mt-3 text-4xl font-black">
                        ...
                        <div className="rounded-xl border border-base-200 bg-base-200/30 p-5">
                            <p className="text-sm font-semibold uppercase tracking-[0.08em] text-base-content/55">Carga por volumen</p>
                            <p className="display-num mt-3 text-4xl font-black">{templateSummary.totalVolume.toFixed(1)}</p>
                            <p className="mt-2 text-sm text-base-content/60">Metros cúbicos consolidados en el periodo cargado.</p>
                        </div>
                        <div className="rounded-xl border border-base-200 bg-base-200/30 p-5">
                            <p className="text-sm font-semibold uppercase tracking-[0.08em] text-base-content/55">Ritmo de traslado</p>
                            <p className="display-num mt-3 text-4xl font-black">{templateSummary.averageTravel.toFixed(1)} h</p>
